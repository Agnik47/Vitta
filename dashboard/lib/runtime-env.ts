...
// from a plain script). `lib/gate-cli.ts` already hit this exact problem with the RAZORPAY_* vars and
...
function loadEnvFiles(): Record<string, string> {
  ...
  for (const file of candidates) {
    ...
    Object.assign(merged, parseEnvFile(readFileSync(file, "utf-8")));
  }
  cached = merged;
  return merged;
}
