...
// merchant" run into a blunt FAILED.
...
export function deriveJobState(events: AgentLine[], current: PurchaseJobState = "PENDING"): PurchaseJobState {
    for (let i = events.length - 1; i >= 0; i--) {
        ...
        if (ev.type === "result") {
            ...
            // ok:true with no receiptId and not awaiting confirmation is the proof:'none' hand-off
            // case (a merchant with no order-placing command at all — the gate approved and drove
            // the browser as far as it can, but there is nothing for the merchant to confirm to us).
        ...
        if (ev.type === "step") {
            ...
            // 'commit' is only ever reached for a genuine policy DENY/STEP_UP now — a real ALLOW
            // always produces an 'authorize' step instead (see PurchaseAgent.ts's run()).
            if (ev.step === "commit") {
                if (ev.status === "running") return "WAITING_GATE";
                if (ev.status === "failed") return "FAILED";
            }
