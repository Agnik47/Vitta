...
// or PravaLedger directly — every money-moving or browser-driving step goes through here.
 ...
 *  PravaLedger can reach the Prava sandbox API without requiring the calling shell to
 *  have sourced `.env` first. Errors are swallowed — if `.env` is missing the child process simply
 *  inherits whatever the parent already has, and PravaLedger's own requireEnv() will surface
