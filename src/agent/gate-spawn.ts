...
// or a ledger directly — every money-moving or browser-driving step goes through here.
 ...
 *  RazorpayLedger can reach Razorpay's test API without requiring the calling shell to
 *  have sourced `.env` first. Errors are swallowed — if `.env` is missing the child process simply
 *  inherits whatever the parent already has, and RazorpayLedger's own credential check will surface
