...
**Last updated:** 2026-07-29 (later still), Agent A (`gate run`/`gate fund` wired to the real `Ledger`/`execute()`/receipt chain in `src/cli/gate.ts` — Phase 1f now fully done, not partial)

---

## At a glance

- **Completed:** Phase 0, 1a, 1b, 1e, 1f (all Agent A, 1f now fully done — see below), Phase 1d's `manifest.ts` and Phase 1h (Agent B, fully done), Phase 1c (real provisioning by Agent A + real `Ledger`/`DodoCreditLedger` implementation by Agent B, per direct user instruction — see `02-DECISIONS.md` ADR-006).
- **Active:** `gate run`/`gate fund` are now real code in `src/cli/gate.ts` (Agent A) — both instantiate `DodoCreditLedger` for real and call `decide()`/`execute()`/`buildAndSignReceipt()`. `npx tsc --noEmit` clean, 45/45 tests still passing. The only remaining blocker anywhere is webcmd browser connectivity — B-002 is Agent B's machine (Cloak runtime connects, Connectivity check fails); separately, Agent A's machine doesn't have `webcmd` installed at all yet, so a live Beat 1-6 rehearsal isn't possible on either machine as of this update.
- **Pending:** A live rehearsal of Beats 1-6 on some machine with working webcmd (1g), 1d's `execute()` verification (Agent B, blocked on B-002), 5.
...
| 1g — Full end-to-end run | Agent A | ❌ Blocked | Code side is done — `gate run`/`gate fund` are real and type-check clean. Blocked purely on a working `webcmd` install + passing `doctor` connectivity check somewhere: not true on Agent B's machine (B-002) and `webcmd` isn't even installed on Agent A's. Needs a machine where webcmd's browser bridge actually works before Beats 1-6 can be rehearsed for real. |
...
| 3 — 1c+1d → 1f | Agent A wiring `gate run`/`gate fund` | ✅ Done | `gate run`/`gate fund` wired to the real `DodoCreditLedger` and committed. `execute()` itself is still unverified live (B-002 / no webcmd on Agent A's machine), so a full real `gate run` demo still can't be rehearsed yet even though the code is done. |
...
| 5 — 1f → 1g | Real end-to-end demo run | ⏳ Not reached | 1f is done. Waiting on a machine with a working `webcmd` install (passing `doctor`'s connectivity check) to actually rehearse Beats 1-6 — not yet true anywhere. |
