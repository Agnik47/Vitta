...
**Status, 2026-07-29 (later still): done.** All three concept pages now have real mocked content, built per `dashboard/DESIGN.md`'s token system and the `dataviz`/`frontend-design` skills. `recharts@3.10.1` installed (v3, not v2 — v1/v2 are unmaintained upstream and this was a fresh install with no v2 code to preserve; confirmed clean against React 19).

- [x] **Marketplace comparison view** (`/concept/compare`) — Blinkit/Zepto/BigBasket/Instamart side by side for "Aashirvaad Atta 5kg ×2" (listed price, delivery/platform fees, discount, final checkout cost, ETA), cheapest flagged "Best value" with the oxide accent. One real anchor kept in: Blinkit's ₹476 final cost matches the actual Beats 5-8 receipt — the other three merchants' figures are representative, not scraped.
- [x] **Shopping Rule Builder concept** (`/concept/rules`) — a form (product, target price, budget, merchant toggles, expiry, purchase limit, manual-review/auto-buy segmented toggle) that on submit renders a dashed-border "would-be mandate" preview (same field shape as the real `Mandate` type) — entirely local `useState`, never a network write, explicit copy pointing back to the real `gate mandate create` CLI command.
- [x] **Activity timeline** (`/concept/timeline`) — a 6-stage pipeline stepper (Search/Compare/Checkout as permanent concept-only stages; Mandate approval/Payment/Receipt wired to the real `/api/events`/`/api/receipts` routes via the existing polling hooks, each stage tagged "Real" or "Sample" — never color-alone) plus a `recharts` bar chart blending real receipt totals with 3 labeled sample projections. Real bars use the app's existing `allow` token; sample bars use a diagonal SVG hatch texture (not just a lighter color) as the CVD-safe secondary encoding the `dataviz` skill calls for. On this machine (0 real receipts locally), the chart correctly falls back to sample-only with an honest inline note rather than looking broken or empty.
- [x] Every concept screen gets a visible, unmissable label — `ConceptPreviewBadge` component (pre-existing) plus each page's own footer note is explicit about which numbers are real vs. representative.
- [x] Concept-layer work didn't start before Phase 1 (Beats 5-8) — Agent B's real rehearsal landed first, this session came after.
- **Known gap, carried forward from Phase 2:** not visually verified in an actual browser this session either — `mcp__claude-in-chrome__tabs_context_mcp` reported the Chrome extension not connected at all (a step further than Phase 2's `resize_window`-didn't-resize gap). Verified instead via `tsc --noEmit`/`lint`/`build` all clean, real server-rendered HTML content checked via `curl` against a real `npm run start` on port 3100 (all 6 routes 200, no SSR error markers, clean server log), and root `src/`'s 45/45 tests + `tsc --noEmit` re-confirmed unaffected. A real pixel-level check (including the narrow-viewport responsive check still open from Phase 2) is still worth doing before the live demo if browser tooling becomes available.
