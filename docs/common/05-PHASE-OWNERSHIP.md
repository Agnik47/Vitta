...
**2026-07-29, later still — final-stretch task split, see `09-HACKATHON-WOW-PLAN.md` and ADR-010:** all of Phase 1 (0 through 1h) is now done — this section describes the build-phase split, which is complete history at this point. For the remaining hackathon-prep work, the split is: **Agent B runs the real Beats 5-8 rehearsal** (Plan Phase 1 — squarely inside Agent B's existing `src/webcmd/`/I/O-rails ownership, no reassignment). **Agent A takes the dashboard's presentation layer** (Plan Phase 2/3 — `dashboard/app/**/page.tsx`, `dashboard/components/**`, `dashboard/app/globals.css` only) — a scoped, temporary exception to the "`dashboard/` is Agent B's" rule below, per direct user instruction (ADR-010). `dashboard/lib/*` and `dashboard/app/api/**` remain Agent B's throughout — Agent A doesn't touch those without flagging first, same as always.
